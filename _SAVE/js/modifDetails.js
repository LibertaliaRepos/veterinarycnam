...
if (detailsForm != null) {
    ...
    // Element image permettant de prévisualiser l'image choisi
    var preview = document.getElementById('picturePreview');
    
    
    function srcPreview(event) {
    	preview.src = URL.createObjectURL(event.target.files[0]);
    }
    
    // Affichage d'une preview de l'image choisi
    if (detailsinputFile.addEventListener) {
        detailsinputFile.addEventListener('change', srcPreview,false);
    } else {
        detailsinputFile.attachEvent('onchange', srcPreview);
    }
    ...
    if (delDetailsImg.addEventListener) {
        delDetailsImg.addEventListener('change', disableImg, false);
    } else {
        delDetailsImg.attachEvent('onchange', disableImg);
    }
}